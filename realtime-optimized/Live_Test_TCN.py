...
def stream_infer(frame_rgb: np.ndarray, st: dict):
    ...
    cv2.putText(
        overlay_bgr,
        f"Stable: {stable_name} | prob={stable_prob:.3f}",
        (20, 55),
    ...
    cv2.putText(
        overlay_bgr,
        f"Window: {len(st['buf'])}/{SEQ_LEN} | Raw: {raw_name} ({raw_prob:.3f})",
        (20, 95),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (200, 200, 200),
    ...
    cv2.putText(
        overlay_bgr,
        f"FPS: {st['fps']:.1f} | device: {device.type}",
        (20, 125),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.65,
        (200, 200, 0),
...
with gr.Blocks(title="Live Gesture - Causal TCN") as demo:
    ...
    with gr.Row():
        cam = gr.Image(sources=["webcam"], streaming=True, type="numpy", label="Webcam (streaming)")
    ...
    cam.stream(
        fn=stream_infer, inputs=[cam, state], outputs=[cam, out_text, state], show_progress=False
    )

    btn_reset.click(fn=reset_state, inputs=None, outputs=[state, status], show_progress=False)
